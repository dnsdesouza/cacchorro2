...
document.addEventListener('DOMContentLoaded', function() {
    ...
    fetch(apiUrl)
      ...
      .then(data => {
        if (data.status === 'success') {
          ...
          mensagem.textContent = 'Falha ao carregar as imagens. Tente selecionar a raça.';
        }
      })
      .catch(error => {
  ...
  function exibirImagem(indice) {
    // Limpa o conteúdo atual da galeria pra garantir q a galeria esteja vazia na hora de salvar imagem na galeria
    galeria.innerHTML = '';
    ...
    try {
      ...
      cachorrosSalvos.forEach(cachorro => {
       ...
       // Criando um cartão para exibir cada cachorro salvo
        ...
        card.setAttribute('data-index', cachorrosSalvos.indexOf(cachorro));

        // Adicionando um evento de clique ao cartão do cachorro
        card.addEventListener('dblclick', function() {
          const index = parseInt(card.getAttribute('data-index'));
          const cachorroSelecionado = cachorrosSalvos[index];
          removerCachorroDoLocalStorage(cachorroSelecionado);
          exibirCachorrosSalvos();
        });
        ...
        const botaoRemover = document.createElement('X');
